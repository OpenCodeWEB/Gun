/**
 * gunx.js — GunX client SDK (fork-bundled copy).
 *
 * This is the same SDK published in the OpenCodeWEB/GunX project
 * (https://github.com/OpenCodeWEB/GunX), bundled into this fork so it can be
 * loaded as `require('gun/lib/gunx')` and used through the serverless
 * gunx.pages.dev relay with zero relay-server setup. Wraps a gun instance and
 * adds:
 *
 ...
 *
 * Usage (Node, from this fork):
 *   const Gun = require('gun');
 *   const GunX = require('gun/lib/gunx');
...
// Fork convenience: also expose GunX on the Gun constructor when loaded from
// within this fork (Node), so `Gun.GunX` works like `Gun.SEA` does.
try {
  var __gx = require("../gun");
  if (__gx && typeof __gx === "function" && !__gx.GunX) __gx.GunX = module.exports;
} catch (e) {
  /* noop */
}
